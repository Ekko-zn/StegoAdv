...
def f(x,model,labels):
    ...
    return torch.clamp(1*(j-i), min=-1)
...
def zn_attack(images,labels,order,c=50,maxstep=5000):
    if order == 3:
        ...
        for ii in range(maxstep):
            ...
            adv_img = Embbed(temp1).view(-1, 1, img_size, img_size)*1 + \
                Embbed(temp2).view(-1, 1, img_size, img_size)*2 + Embbed(temp3).view(-1, 1, img_size, img_size)*3 + test_image
            ...
            advLOSS = torch.sum(f(adv_img/255,model,labels))
            loss = advLOSS + c*losshpf
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            tmp = torch.sum(f(adv_img/255,model,labels)).item()         
            if torch.sum(f(adv_img/255,model,labels)).item() < -0.01*batch_size:
        ...
        for ii in range(maxstep):
            ...
            tmp = torch.sum(f(adv_img/255,model,labels)).item()
            # if ii % 20 == 0:
            #     print('step:{} order:{}  tmp:{} hpf:{}'.format(ii,order,tmp,losshpf.item()))        
            if torch.sum(f(adv_img/255,model,labels)).item() < -0.01*batch_size:
                tmp = adv_img.clone().detach()
                tmp[tmp>255]=255
                tmp[tmp<0]=0
